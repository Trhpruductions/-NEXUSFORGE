        ...
        <div className="nexus-display-panel mb-5 rounded-[28px] p-5">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-sm text-slate-400">The ranking feed refreshes live with reputation, streaks, and medal momentum.</p>
            </div>
      ...
      {isSignedIn ? (
        <>
          <div className="nexus-display-panel rounded-[28px] p-1.5">
            <div className="flex gap-1 rounded-[24px] border border-white/10 bg-[#11151e] p-1 shadow-[0_12px_30px_rgba(15,23,42,0.05)]">
          ...
          {loading ? (
            <div className="nexus-display-panel rounded-[28px] p-5 text-center text-sm text-slate-400">Loading leaderboard...</div>
          ) : (
              ...
              {leaderboard.map((entry, index) => {
                ...
                return (
                  <Link
                    ...
                    className="nexus-interactive-card flex items-center gap-4 rounded-[24px] border border-white/10 bg-[#11151e] px-4 py-4 shadow-[0_14px_36px_rgba(15,23,42,0.06)]"
                  >
                    <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-gradient-to-br text-sm font-bold text-white ${getMedalColor(entry.premiumTier)}`}>
                      {index + 1}
                    </div>
                    ...
                    {entry.avatar ? (
                      ...
                      <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-white/10 bg-[#11151e] text-sm font-bold text-slate-300">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-1 flex flex-wrap items-center gap-2 text-xs">
                        {entry.clanTag ? <p className="text-amber-200">[{entry.clanTag}]</p> : null}
                        <span className="text-slate-500">Rank #{index + 1}</span>
                      </div>
                    </div>

                    <div className="shrink-0 rounded-[20px] border border-white/10 bg-[#0d1119] px-3 py-2 text-right">
                      {activeTab === "reputation" ? <span className="text-lg font-bold text-amber-200">{entry.reputation ?? 0}</span> : null}
                      {activeTab === "streaks" ? (
                        <div>
                          <span className="text-lg font-bold text-amber-200">{entry.corePlusBoostLevel ?? 0}</span>
