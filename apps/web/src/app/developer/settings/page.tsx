...
export default function DeveloperSettingsPage() {
  ...
  return (
    ...
    >
      {!isSignedIn ? (
        <div className="rounded-[28px] border border-white/10 bg-[#11151e] p-8 text-slate-400 shadow-[0_24px_60px_rgba(15,23,42,0.08)]">Please sign in to manage your developer credentials.</div>
      ) : (
        <div className="grid gap-6 lg:grid-cols-[1.3fr_0.9fr]">
          <div className="space-y-6">
            <section className="rounded-[28px] border border-white/10 bg-[#11151e] p-6 shadow-[0_24px_60px_rgba(15,23,42,0.08)] backdrop-blur-xl">
              ...
              <p className="mt-3 text-sm leading-7 text-slate-400">Generate a key for your integrations and keep your secret safe. You will only see the secret once.</p>
            ...
            <section className="rounded-[28px] border border-white/10 bg-[#11151e] p-6 shadow-[0_24px_60px_rgba(15,23,42,0.08)] backdrop-blur-xl">
              ...
              <p className="mt-3 text-sm leading-7 text-slate-400">Register a client for OAuth redirect flows and keep your credentials secure. The secret is revealed only once.</p>
                  ...
                  <div className="rounded-[24px] border border-emerald-400/40 bg-emerald-500/10 p-4 text-sm text-emerald-800">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        ...
                        <p className="mt-2 break-all text-white">{createdOauthSecret}</p>
                        <p className="mt-2 text-xs text-slate-500">Save this secret now. It will not be shown again.</p>
                      </div>
                      <button
                        type="button"
            ...
            <div className="mt-6 space-y-3">
              {keysQuery.isLoading ? (
                ...
                keysQuery.data.keys.map((key) => (
                  <div key={key.id} className="rounded-[24px] border border-white/10 bg-[#0d1119] p-4">
                    <div className="flex flex-wrap items-start justify-between gap-4">
                      ...
                      <div className="flex items-center gap-2">
                        <span className={`rounded-full px-3 py-1 text-xs uppercase tracking-[0.24em] ${key.enabled ? "border border-emerald-200 bg-emerald-50 text-emerald-700" : "border border-white/10 bg-amber-400 text-slate-400"}`}>
            ...
            <div className="mt-6 space-y-3">
              {oauthClientsQuery.isLoading ? (
                ...
                oauthClientsQuery.data.clients.map((client) => {
                  ...
                  return (
                    <div key={client.id} className="rounded-[24px] border border-white/10 bg-[#0d1119] p-4">
                      {isEditing ? (
                        <div className="space-y-4">
                          <label className="grid gap-2 text-sm text-slate-400">
                            ...
                            <input
                              ...
                              className="w-full rounded-[20px] border border-white/10 bg-amber-400 px-4 py-3 text-sm text-slate-950 outline-none focus:border-sky-300"
                            />
                          </label>
                          <label className="grid gap-2 text-sm text-slate-400">
                            Redirect URIs
                          ...
                          <div className="flex flex-wrap items-center gap-3">
                            <button
                              ...
                              className="inline-flex h-11 items-center justify-center rounded-full bg-sky-500 px-5 text-sm font-semibold text-white transition hover:bg-sky-400 disabled:cursor-not-allowed disabled:opacity-60"
                            >
                            ...
                            <button
                              ...
                              className="inline-flex h-11 items-center justify-center rounded-full border border-white/10 bg-amber-400 px-5 text-sm font-semibold text-slate-950 transition hover:bg-[#0d1119]"
                            >
                              Cancel
                            </button>
                          </div>
                        ...
                        <>
                          <div className="flex flex-wrap items-start justify-between gap-4">
                            ...
                            <div className="flex flex-wrap items-center gap-2">
                              <span className={`rounded-full px-3 py-1 text-xs uppercase tracking-[0.24em] ${client.enabled ? "border border-emerald-200 bg-emerald-50 text-emerald-700" : "border border-white/10 bg-amber-400 text-slate-400"}`}>
                              ...
                              <button
                                ...
                                className="inline-flex h-10 items-center justify-center rounded-full border border-emerald-400/40 bg-emerald-500/10 px-4 text-xs font-semibold text-emerald-200 transition hover:bg-emerald-100"
                              >
                                Rotate secret
                              </button>
                              <button
          ...
          {rotatedOauthSecret ? (
              <section className="rounded-[28px] border border-emerald-400/40 bg-emerald-500/10 p-6 text-white shadow-[0_24px_60px_rgba(15,23,42,0.08)]">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <p className="text-sm font-semibold text-emerald-900">New client secret</p>
                  <p className="mt-3 break-all text-white">{rotatedOauthSecret}</p>
                  <p className="mt-2 text-xs text-slate-500">Save this secret now. It will not be shown again.</p>
                </div>
                <button
                  type="button"
