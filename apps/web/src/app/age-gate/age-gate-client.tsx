...
export function AgeGateClient() {
  ...
  return (
    <section className="rounded-[28px] border border-white/10 bg-[#0d1119] p-8 shadow-[0_18px_48px_rgba(15,23,42,0.08)]">
      <div className="space-y-5">
        ...
        {status === "error" ? (
          <div className="rounded-[20px] border border-rose-400/40 bg-rose-500/10 p-4 text-sm text-rose-200">
            {errorMessage ?? "Verification failed. Please try again."}
          </div>
        ) : null}

        {status === "denied" ? (
          <div className="rounded-[20px] border border-white/10 bg-[#0d1119] p-4 text-sm text-slate-300">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <button
            ...
            className="inline-flex h-14 items-center justify-center rounded-full bg-amber-500 px-6 text-sm font-semibold text-white transition hover:bg-amber-400 disabled:cursor-not-allowed disabled:opacity-60"
          >
          ...
          <button
            ...
            onClick={deny}
            disabled={status === "submitting" || status === "denied"}
            className="inline-flex h-14 items-center justify-center rounded-full border border-white/10 bg-amber-400 px-6 text-sm font-semibold text-slate-300 transition hover:bg-[#0d1119] disabled:cursor-not-allowed disabled:opacity-60"
          >
            I am under 18
          </button>
        </div>

        <div className="rounded-[20px] border border-white/10 bg-[#0d1119] p-4 text-sm text-slate-400">
