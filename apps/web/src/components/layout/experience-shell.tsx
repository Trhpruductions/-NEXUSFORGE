...
export function ExperienceShell({
  ...
  return (
    <div className="nexus-shell">
      <div className={`nexus-shell-inner ${maxWidthClassName} space-y-5`}>
        {/* Page header */}
        <header className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-[0.28em] text-cyan-400">{eyebrow}</p>
        ...
        {metrics.length ? (
          <div className="grid gap-3 sm:grid-cols-3">
            {metrics.map((metric) => (
              <div key={metric.label} className="nexus-metric-card rounded-2xl px-4 py-3">
