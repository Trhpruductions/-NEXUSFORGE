...
export function AdminDashboard() {
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[1fr_1.4fr]">
      <div className="nexus-panel-glass flex flex-wrap items-center gap-3 rounded-[28px] border border-white/10 bg-[#0d1119] p-4 shadow-[0_28px_70px_rgba(0,0,0,0.35)] lg:col-span-2">
        ...
        <Link href="/admin/age-gate-review" className="rounded-full border border-white/10 bg-[#0d1119] px-3 py-2 text-sm font-semibold text-white transition hover:border-amber-400/60 hover:bg-[#0d1119]/95">
          Gate Review
        </Link>
        <Link href="/notifications" className="rounded-full border border-white/10 bg-[#0d1119] px-3 py-2 text-sm font-semibold text-white transition hover:border-amber-400/60 hover:bg-[#0d1119]/95">
          Alerts
        </Link>
        <Link href="/app" className="rounded-full border border-white/10 bg-[#0d1119] px-3 py-2 text-sm font-semibold text-white transition hover:border-amber-400/60 hover:bg-[#0d1119]/95">
          Back to App
        </Link>
      ...
      <section className="nexus-display-panel relative overflow-hidden rounded-[28px] p-5">
        ...
        <div className="grid grid-cols-2 gap-3 text-sm text-slate-300">
          <div className="glass-cut rounded-xl border border-white/10 p-3">Users: {summaryQuery.data?.users ?? 0}</div>
          <div className="glass-cut rounded-xl border border-white/10 p-3">Forges: {summaryQuery.data?.forges ?? 0}</div>
          <div className="glass-cut rounded-xl border border-white/10 p-3">Messages: {summaryQuery.data?.messages ?? 0}</div>
          <div className="glass-cut rounded-xl border border-white/10 p-3">Notifications: {summaryQuery.data?.notifications ?? 0}</div>
          <div className="glass-cut rounded-xl border border-white/10 p-3">Pending Friend Requests: {summaryQuery.data?.pendingFriends ?? 0}</div>
          <div className="glass-cut rounded-xl border border-white/10 p-3">Active Subscriptions: {revenueQuery.data?.revenue.activeSubscriptions ?? 0}</div>
        </div>

        <div className="mt-4 grid gap-2 text-sm text-slate-300">
          <div className="glass-cut rounded-xl border border-white/10 p-3">
      ...
      <section className="nexus-panel rounded-[28px] p-5">
        <div className="mb-4 flex items-start justify-between gap-3">
          ...
          <div className="rounded-full border border-white/10 bg-[#0d1119] px-3 py-1 text-[10px] uppercase tracking-[0.18em] text-slate-400">
            {usersQuery.data?.users.length ?? 0} users
          </div>
        </div>
        <div className="space-y-2">
          {usersQuery.data?.users.map((entry) => (
            <div key={entry.id} className="nexus-interactive-card flex flex-col items-stretch gap-3 rounded-[24px] border border-white/10 bg-[linear-gradient(155deg,rgba(15,23,42,0.96),rgba(8,47,73,0.16))] px-4 py-4 text-sm text-slate-300 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                <select
                  ...
                  className="h-9 min-w-0 rounded-lg border border-white/10 bg-[#0d1119] px-2 text-xs text-white sm:min-w-[140px]"
                ...
                <select
                  ...
                  className="h-9 min-w-0 rounded-lg border border-white/10 bg-[#0d1119] px-2 text-xs text-white sm:min-w-[128px]"
      ...
      <section className="nexus-panel rounded-[28px] p-5 lg:col-span-2">
          ...
          <table className="w-full min-w-[720px] border-collapse text-left text-sm">
            ...
            <tbody>
              {(revenueQuery.data?.featureRevenue ?? []).map((row) => (
                <tr key={row.featureCode} className="border-b border-slate-900/10/70 text-slate-300 transition-colors hover:bg-[#11151e]">
                  <td className="py-2 pr-3 font-medium text-white">{row.featureLabel ?? PAID_FEATURE_DISPLAY_LABELS[row.featureCode]}</td>
        ...
        <div className="mt-3 grid gap-2 md:grid-cols-4">
          <div className="glass-cut rounded-xl border border-white/10 p-3 text-sm text-slate-300">{CORE_PLUS_TIER_DISPLAY_LABELS.CORE}: {revenueQuery.data?.tierDistribution.CORE ?? 0}</div>
          <div className="glass-cut rounded-xl border border-white/10 p-3 text-sm text-slate-300">{CORE_PLUS_TIER_DISPLAY_LABELS.PLUS}: {revenueQuery.data?.tierDistribution.PLUS ?? 0}</div>
          <div className="glass-cut rounded-xl border border-white/10 p-3 text-sm text-slate-300">{CORE_PLUS_TIER_DISPLAY_LABELS.ELITE}: {revenueQuery.data?.tierDistribution.ELITE ?? 0}</div>
          <div className="glass-cut rounded-xl border border-white/10 p-3 text-sm text-slate-300">{CORE_PLUS_TIER_DISPLAY_LABELS.INFINITE}: {revenueQuery.data?.tierDistribution.INFINITE ?? 0}</div>
        </div>
      </section>
      ...
      <section className="nexus-panel rounded-[28px] p-5 lg:col-span-2">
        ...
        {aiInsightsQuery.isError ? (
          ...
          <div className="grid gap-3 md:grid-cols-3">
            <div className="glass-cut rounded-xl border border-white/10 p-4 text-slate-300">
            ...
            <div className="glass-cut rounded-xl border border-white/10 p-4 text-slate-300 md:col-span-3">
            ...
            <div className="md:col-span-3 rounded-xl border border-white/10 bg-[#0d1119] p-4 text-sm text-slate-300">
              ...
              <div className="grid gap-2 md:grid-cols-3">
                {(aiInsightsQuery.data?.insights.automationActions ?? []).map((action) => (
                  <div key={action} className="rounded-lg border border-white/10 bg-[#0d1119] px-3 py-2">
                    {action}
                  </div>
                ))}
              </div>
            ...
            <div className="md:col-span-3 rounded-xl border border-white/10 bg-[#0d1119] p-4 text-sm text-slate-300">
              ...
              <div className="grid gap-2 md:grid-cols-3">
                {(aiInsightsQuery.data?.insights.recommendedPlaybooks ?? []).map((playbook) => (
                  <div key={playbook.title} className="rounded-lg border border-white/10 bg-[#0d1119] px-3 py-2">
      ...
      <section className="nexus-panel rounded-[28px] p-5 lg:col-span-2">
        ...
        <div className="mb-3 rounded-[20px] border border-white/10 bg-[#0d1119] px-3 py-2 text-xs text-slate-300">
        ...
        {profileToolsStatusQuery.data?.latestJob ? (
          <div className="mb-3 rounded-[20px] border border-white/10 bg-[#0d1119] p-3 text-xs text-slate-300">
            ...
            {latestJobResult ? (
              <div className="mt-3 grid gap-2 md:grid-cols-4">
                <div className="rounded-lg border border-white/10 bg-[#0d1119] px-3 py-2">
        ...
        <div className="grid gap-3 md:grid-cols-3">
          ...
          <div className="flex items-center gap-2 rounded-xl border border-white/10 bg-[#0d1119] px-3 py-2">
            <select
              aria-label="Select user for reputation adjustment"
              title="Select user for reputation adjustment"
              className="h-8 flex-1 rounded border border-white/10 bg-amber-400 px-2 text-xs text-slate-950"
            ...
            <input
              ...
              placeholder="Delta"
              className="h-8 w-20 rounded border border-white/10 bg-amber-400 px-2 text-xs text-slate-950"
      ...
      <section className="nexus-panel rounded-[28px] p-5 lg:col-span-2">
        ...
        <div className="mb-3 grid gap-2 md:grid-cols-2">
          <select
            ...
            className="h-9 rounded border border-white/10 bg-amber-400 px-2 text-xs text-slate-950"
        ...
        <div className="grid gap-2">
          {(profileAuditQuery.data?.logs ?? []).map((entry) => (
            <article key={entry.id} className="rounded-[20px] border border-white/10 bg-[linear-gradient(155deg,rgba(15,23,42,0.96),rgba(8,47,73,0.14))] p-3 text-xs text-slate-300">
          ...
          {profileAuditQuery.data && profileAuditQuery.data.logs.length === 0 ? (
            <p className="rounded-xl border border-white/10 bg-[#0d1119] p-3 text-xs text-slate-400">No profile-tool audit events yet.</p>
          ) : null}
        </div>
      ...
      <section className="nexus-panel rounded-[28px] p-5 lg:col-span-2">
        ...
        <div className="grid gap-3">
            ...
            <article key={entry.id} className="rounded-[20px] border border-white/10 bg-[linear-gradient(155deg,rgba(15,23,42,0.96),rgba(8,47,73,0.14))] p-3 text-xs text-slate-300">
              ...
              <div className="mt-3 grid gap-2">
                <textarea
                  ...
                  className="min-h-[72px] w-full rounded-2xl border border-white/10 bg-amber-400 px-3 py-2 text-xs text-slate-950 focus:border-amber-400 focus:outline-none"
                />
          ...
          {ageGateAuditQuery.data && ageGateAuditQuery.data.logs.length === 0 ? (
            <p className="rounded-xl border border-white/10 bg-[#0d1119] p-3 text-xs text-slate-400">No pending age gate review items found.</p>
          ) : null}
        </div>
      </section>
    </div>
