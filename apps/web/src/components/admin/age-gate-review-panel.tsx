...
export function AgeGateReviewPanel() {
  ...
  return (
    <div className="grid gap-6">
      <div className="nexus-panel rounded-[24px] border border-white/10 bg-[#11151e] p-5 shadow-[0_18px_45px_rgba(15,23,42,0.07)]">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="text-sm uppercase tracking-[0.24em] text-amber-200">Age Gate Review</h2>
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          <label className="block text-xs text-slate-400">
            ...
            <select
              ...
              className="mt-2 w-full rounded-[14px] border border-white/10 bg-[#0d1119] px-3 py-2 text-sm text-white"
            >
      ...
      <div className="grid gap-4">
        {(ageGateAuditQuery.data?.logs ?? []).map((entry) => (
          <article key={entry.id} className="rounded-[22px] border border-white/10 bg-[#11151e] p-5 text-sm text-slate-300 shadow-[0_14px_36px_rgba(15,23,42,0.06)]">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div className="min-w-0">
                <p className="text-amber-200">{entry.action === "verify" ? "Age verification attempt" : "Age verification reject"}</p>
                <p className="mt-1 text-xs text-slate-500">{new Date(entry.createdAt).toLocaleString()}</p>
              </div>
              <div className="flex flex-wrap gap-2 text-xs text-slate-400">
                <span className="rounded-full border border-white/10 bg-[#0d1119] px-2 py-1">{entry.status.toUpperCase()}</span>
                <span className="rounded-full border border-white/10 bg-[#0d1119] px-2 py-1">{entry.risk.level.toUpperCase()}</span>
                <span className="rounded-full border border-white/10 bg-[#0d1119] px-2 py-1">{entry.action}</span>
              </div>
            </div>

            <div className="mt-4 grid gap-3 sm:grid-cols-[1fr_auto]">
              <div className="space-y-2">
                <p className="text-slate-400">Fingerprint: <span className="font-mono text-[11px] text-slate-500">{entry.fingerprint}</span></p>
                <p className="text-slate-400">IP: <span className="text-slate-500">{entry.ip}</span></p>
                <p className="text-slate-400">User Agent: <span className="text-slate-500 break-words">{entry.userAgent}</span></p>
                  ...
                  <div className="space-y-2 text-slate-400">
                    ...
                    <div className="grid gap-1 text-xs text-slate-500 sm:grid-cols-2">
                      {Object.entries(entry.deviceProfile).slice(0, 6).map(([key, value]) => (
                        <p key={key}>
                          <span className="font-semibold text-slate-400">{key}:</span> {String(value)}
                        </p>
                      ))}
                    </div>
                  </div>
            ...
            <div className="mt-4 grid gap-3">
              <textarea
                ...
                className="min-h-[88px] w-full rounded-[14px] border border-white/10 bg-[#0d1119] px-3 py-2 text-sm text-white focus:border-amber-400 focus:outline-none"
              />
              <div className="flex flex-wrap gap-2">
        ...
        {ageGateAuditQuery.data && ageGateAuditQuery.data.logs.length === 0 ? (
          <div className="rounded-[20px] border border-white/10 bg-[#0d1119] p-5 text-sm text-slate-400">
